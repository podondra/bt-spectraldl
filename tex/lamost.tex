...
Convolutional layers are mark as conv3
...
Max-pooling layers with pool size 2, stride 2 and no padding.
Fully-connected layer with 512 units are fc-512
and softmax is the output layer.
\endinsert

\sec Training Details

The process of babysitting the neural network is described in this section.
Dataset split and balancing is introduced.
Then the parameters of training the network are described.
...
The matrix show 92\% accuracy on double-peak spectra
...
(see confusion matrix on left in figure~\ref[confusion-matrices]).
...
Training the neural networks is an optimization problem.
...
Parameter are set to values recommended by its paper
...
Figure~\ref[convnet-training] show the \glref{CNN}'s progress
...
\glref{CNN} (right matrix) performs slightly better then feedforward network.
\endinsert
...
has 2048 \glref{CUDA} cores and 4\,GB of memory.
The two Intel Xeon \glref{CPU}
...
Training on \glref{GPU} is 27 times faster then on \glref{CPU}.
...
may scale to more \glref{GPU} card on demand. \cite[tensorflow]
...
Prediction of {\bf 2\,202\,000} spectra from LAMOST data release 1 has taken
{\bf 1 minute 36 seconds}.
...
\ctable{lrr}{
    \hfil class & number of spectra & candidates \hfil \crl \tskip4pt
    emission    &    158\,115       & 31751 \cr
    absorption  & 1\,898\,095       & \cr
    double-peak &    145\,790       & 47575 \cr
}
\caption/t Counts of LAMOST spectra split according to predicted classes.
The last candidates column count only spectra predicted as emission
or double-peak and fluxes intensities in range from $-5$ to $5$
across all wavelengths.
\endinsert

When visualizing the emission and double-peak spectra from LAMOST a lot of
them are very noisy.
The noisy spectra has huge fluxes in some wavelength thus
suppressing features for example in H$\alpha$ range
(see figure~\ref[lamost-noise]).
Because there are no such spectra in Ondřejov dataset the network
classifies them incorrectly.
Therefore a range for fluxes was empirically chosen from $-5$ to $5$
and only spectra from this range and emission or double-peak class
can be considered as candidates
(for counts see candidates column in table~\ref[prediction-table]).

\midinsert \clabel[lamost-noise]{Noisy LAMOST spectra}
\picw=15cm \cinspic img/lamost-noise.png
\caption/f Plots of two spectra from LAMOST with high fluxes values.
The huge fluxes clear the characteristic shape in H$\alpha$ region.
\endinsert

Candidates visualizations TODO.
