...
where the number 3 means teh size of filter in pixels.
...
Lastly the networks' performance is evalueted.
...
\caption/f Confusion matrix evalueted on validation set
...
In Tensorlow this implements code:

\begtt
import tensorflow.contrib.keras as keras

model = keras.models.Sequential([
    # defition of neural network
])
model.compile(
...
network only 98\%.
Both predicted 99\% of absorpitons correctly.
...
\glref{CNN} (right matrix) performs sligthly better then feedforward network.
\endinsert
...
Recent advance in deep neural network architecture would not be possible
without performance improvement of general purpose \glref{GPU}s. \cite[alexnet]
This thesis makes similar observation.
The convolutional network was experimentaly
trained on \glref{GPU} GeForce GTX 980 and Intel Xeon \glref{CPU}.
...
The Intel Xeon \glref{CPU} E5-2403 1.80\,GHz\urlnote{http://ark.intel.com/products/64615/Intel-Xeon-Processor-E5-2403-10M-Cache-1_80-GHz-6_40-GTs-Intel-QPI}
has 4 cores and 24\,GB of available \glref{RAM}.
